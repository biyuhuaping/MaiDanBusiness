...
-(instancetype)initWithFrame:(CGRect)frame{
    ...
    if(self){
        self.imageV = [[CLLockItView alloc] init];
        if (iPhone6p || iPhone6) {
            self.imageV.frame = CGRectMake(-8, 0, 88, 88);
   
        }else{
            self.imageV.frame = CGRectMake(-8, -8, 68, 68);
...
-(void)drawRect:(CGRect)rect{
    ...
    _imageV.image = [UIImage imageNamed:@"jiesuoquan_1"];
    ...
    if(_selected){
        ...
        //外环：选中
//        [self.imageV circleSelected:ctx rect:rect];
        
        //三角形：方向标识
        [self  directFlag:ctx rect:rect];
        _imageV.image = [UIImage imageNamed:@"jiesuoquan_2"];
...
-(CGRect)selectedRect{
    ...
    if(CGRectEqualToRect(_selectedRect, CGRectZero)){
        ...
        CGFloat selectRectWH = rect.size.width * CoreLockArcWHR + 20;
