...
-(void)drawRect:(CGRect)rect{
    ...
    [_itemViewsM enumerateObjectsUsingBlock:^(CLLockItemView *itemView, NSUInteger idx, BOOL *stop) {
        ...
        CGPoint directPoint = CGPointMake(itemView.center.x, itemView.center.y + 4);
...
-(void)layoutSubviews{
    ...
    if (iPhone6p) {
        itemViewWH = (self.frame.size.width - 5 * marginValue) /3.0f;
    }
    [self.subviews enumerateObjectsUsingBlock:^(UIView *subview, NSUInteger idx, BOOL *stop) {
        ...
        CGRect frame ;
        if (iPhone6p) {
            frame = CGRectMake(x + marginValue/3, y, itemViewWH, itemViewWH);
        }
        else if (iPhone6){
            frame = CGRectMake(x, y - 10, itemViewWH, itemViewWH);
        }
